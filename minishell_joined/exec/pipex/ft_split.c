...
/*   Updated: 2025/02/25 17:25:39 by juan-ant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	meter(const char*s, char c, char**retorno)
{
	...
	while (i < ft_strllen (s))
	{
		if (s[i] != c)
		{
			...
			while (s[i] != c && i < ft_strllen (s))
				i++;
			retorno[a - 1] = ft_subbstr (s, start, i - start);
			if (!(retorno[a - 1]))
			{
				ft_free(retorno, a - 1);
				return (0);
...
int main(void)
{
    char *str = "Este es un test para ft_ssplit_exec";
    char **result;
    int i = 0;

    result = ft_ssplit_exec(str, ' ');
